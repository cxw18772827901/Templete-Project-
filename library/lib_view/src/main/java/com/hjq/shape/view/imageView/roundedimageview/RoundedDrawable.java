...
public class RoundedDrawable extends Drawable {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        if (mShape == Shape.CIRCLE) {
            ...
            float min = Math.min(w, mBounds.height());
            setRect(w, h, min, mDrawableRect);
            canvas.drawOval(mDrawableRect, mBitmapPaint);
            if (mBorderWidth > 0) {
                setRect(w, h, min, mBorderRect);
    ...
    private void setRect(float w, float h, float min, RectF mDrawableRect) {
        ...
        mDrawableRect.bottom = h - (h - min) / 2 - mBorderWidth / 2;
    }
    ...
    public RoundedDrawable setOval(int shape) {
        mShape = shape;
        return this;
    }
