...
public class ProgressDrawable extends PaintDrawable implements Animatable, ValueAnimator.AnimatorUpdateListener {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        final float r = Math.max(1f, width / 40f);

        if (mWidth != width || mHeight != height) {
            mPath.reset();
            mPath.addCircle(width - r, height / 2f, r, Path.Direction.CW);
            mPath.addRect(width - 7 * r, height / 2f - r, width - r, height / 2f + r, Path.Direction.CW);
            mPath.addCircle(width - 7 * r, height / 2f, r, Path.Direction.CW);
            mWidth = width;
            mHeight = height;
        }
