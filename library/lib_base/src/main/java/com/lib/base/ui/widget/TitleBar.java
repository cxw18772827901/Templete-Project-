...
import static com.lib.base.ui.activity.AppTheme.THEME_NONE;


/**
 * 标题TitleBar,可以设置主题和对应主题下返回按钮,中间标题,右边按钮(支持pop列表)等样式属性:
 * 1.默认主题有蓝色(背景)和白色(背景)两个颜色,可以配置多个,对应{@link AppTheme}中配置;
...
public class TitleBar extends FrameLayout {
    ...
    public TitleBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (barInitModel == -1 && isInEditMode()) {
            //开启预览模式
            result = initView(context, AppTheme.THEME_BLUE);
        } else if (barInitModel == 0) {
            //blue title theme
            result = initView(context, AppTheme.THEME_BLUE);
        } else if (barInitModel == 1) {
            //white title theme
            result = initView(context, AppTheme.THEME_WHITE);
        } else {
            return;
        }
    ...
    public boolean initView(Context context, int theme) {
        ...
        int layoutId = AppTheme.THEME_BLUE == theme ? R.layout.blue_title_layout : R.layout.white_title_layout;
