...
public final class GlideConfig extends AppGlideModule {
     ...
     * 本地图片缓存文件最大值1g
     */
    private static final int IMAGE_DISK_CACHE_MAX_SIZE = 1024 * 1024 * 1024;
    ...
    public void applyOptions(@NonNull Context context, @NonNull GlideBuilder builder) {
        ...
        builder.setDefaultRequestOptions(new RequestOptions()
                // 设置默认加载中占位图
                .placeholder(R.drawable.img_drawable)
                // 设置默认加载出错占位图
                .error(R.drawable.img_drawable));
    }

    @Override
    public void registerComponents(@NonNull Context context, @NonNull Glide glide, @NonNull Registry registry) {
        // Glide 默认使用的是 HttpURLConnection 来做网络请求，这里切换成更高效的 OkHttp
        registry.replace(GlideUrl.class, InputStream.class, new OkHttpLoader.Factory((Call.Factory) RetrofitHelper.getInstance().getOkHttpClient()));
    }
